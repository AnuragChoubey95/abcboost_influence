...
num_columns=$(head -n 1 ../influence_scores/${substring}.test.csv_regression_J20_v0.1_p2BoostIn_Influence.csv | awk -F',' '{print NF}')
...
for index in "${indices[@]}"; do
    ...
    python3 rank_train_samples.py ../influence_scores/${substring}.test.csv_regression_J20_v0.1_p2BoostIn_Influence.csv ${substring}_ranked $index

    # Rank train samples using LCA influence file
    python3 rank_train_samples.py ../influence_scores/${substring}.test.csv_regression_J20_v0.1_p2LCA_Influence.csv ${substring}_ranked $index
