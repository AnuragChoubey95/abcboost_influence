...
indices=($(python3 -c "import random; print(' '.join(map(str, random.sample(range(0, 4000), 20))))"))
...
for index in "${indices[@]}"; do
    ...
    python3 rank_train_samples.py ${substring}.test.csv_regression_J20_v0.1_p2BoostIn_Influence.csv ${substring}_ranked $index

    # Rank train samples using LCA influence file
    python3 rank_train_samples.py ${substring}.test.csv_regression_J20_v0.1_p2LCA_Influence.csv ${substring}_ranked $index
    ...
    for percentage in "${percentages[@]}"; do
        ...
        python3 generate_custom_data.py ${substring}_ranked_column_${index}_BoostIn.csv ${substring}.train.csv ${percentage}%

        # Generate custom data for LCA influence file
        python3 generate_custom_data.py ${substring}_ranked_column_${index}_LCA.csv ${substring}.train.csv ${percentage}%
    done
    echo "-----------------------------------------------------------------------------------------------------------------"
done
